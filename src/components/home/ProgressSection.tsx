...
function RadialRing({ percent, subjectId, size = 96, stroke = 8 }: { percent: number; subjectId: SubjectId; size?: number; stroke?: number }) {
...
function SubjectCard({ subjectId, label, description, href, completed, total }: SubjectCardProps) {
    ...
    <div className="group bg-app relative flex flex-col items-center gap-4 rounded-3xl border border-border p-6 text-center transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-muted text-[10px] font-bold tracking-wide uppercase">مكتمل</span>
        </div>
      </div>
      ...
      <div className={cn("flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-bold", tw.bgSoft, tw.text)}>
        {completed} / {total} درس
      </div>

      <Link href={href} className="btn-luxury flex items-center justify-center gap-1.5 py-1 w-full">
        <span className={cn("text-sm font-semibold transition-all duration-200 group-hover:gap-2", tw.text)}>{!percent ? "ابدأ الآن" : percent === 100 ? "مكتمل" : "تابع تقدمك"}</span>
  ...
  return (
    <div className="bg-app rounded-3xl border border-border p-6">
      ...
      <div className="bg-soft h-3 w-full overflow-hidden rounded-full">
        <div className="h-full rounded-full bg-linear-to-r from-accent to-indigo-400 transition-all duration-700" style={{ width: `${percent}%` }} />
      </div>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {SUBJECTS_META.map((s) => {
          ...
          return <SubjectCard key={s.id} subjectId={s.id} label={s.label} description={s.description} href={`/courses/${s.id}`} completed={completed} total={total} />;
        })}
      </div>
    </div>
  );
